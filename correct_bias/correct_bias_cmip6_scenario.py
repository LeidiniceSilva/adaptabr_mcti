...
def import_observed(var_name):
    ...
    file_name = "{0}/obs/{1}_19860101-20051231_BR-DWGD_UFES_UTEXAS_v_3.2.2_lonlat.nc".format(dataset_dir, var_name)
...
def import_simulated(model_name, var_name):
	...
	file_model = "{0}/{1}_br_day_NorESM2-MM_historical_r1i1p1f1_gn_19860101-20051231_lonlat.nc".format(dir_model, var_name)
...
def trend(dataset, fit_params_hist, th_distrib_hist, fit_params_fut, th_distrib_fut):

    k, s, loc, scale = fit_params_hist
    k_f, s_f, loc_f, scale_f = fit_params_fut
    
    # Apply cumulative distribution function with future params and distribution
    cdf_fut = th_distrib_fut.cdf(tmax_fut, k=k_f, s=s_f, loc=loc_f, scale=scale_f)
    
    # Then apply inverse cdf (or ppf) with historical fitted params and distribution
    trend = tmax_fut/(th_distrib_hist.ppf(cdf_fut, k=k, s=s, loc=loc, scale=scale))
    
    return trend
    
     
def quantile_mapping(dataset, fit_params_fut, th_distrib_fut, fit_params_obs, th_distrib_obs, trend):
...
simulated = import_simulated(cmip6[mdl][0], var_cmip6)
lat_array, lon_array, projected = import_projected(cmip6[mdl][0], experiment, var_cmip6, cmip6[mdl][1], dt)
...
proj_k, proj_s, proj_loc, proj_scale = st.mielke.fit(projected)
proj_params = proj_k, proj_s, proj_loc, proj_scale

print(obs_k, obs_s, obs_loc, obs_scale)
print(sim_k, sim_s, sim_loc, sim_scale)
print(proj_k, proj_s, proj_loc, proj_scale)

# Apply trend function
trend_data = trend(projected, sim_params, st.mielke, proj_params, st.mielke)

# Apply quantile mapping function
corrected_data = quantile_mapping(projected, proj_params, st.mielke, obs_params, st.mielke, trend_data)
