...
model_names = [
    ...
    "CNRM-ESM2-1", "GFDL-ESM4", "INM-CM4-8", "INM-CM5-0", "KIOST-ESM",
    "MIROC6", "MIROC-ES2L", "MPI-ESM1-2-HR", "MPI-ESM1-2-LR", "MRI-ESM2-0",
    "NESM3", "NorESM2-MM"
]

n_models = len(model_names)
colors = plt.cm.jet(np.linspace(0, 1, n_models))
...
region_ranks = {
    'NAMZ': cri_namz_vars_rank,
    'SAMZ': cri_samz_vars_rank,
    'NEB': cri_neb_vars_rank,
    'SAM': cri_sam_vars_rank,
    'LPB': cri_lpb_vars_rank,
}


def add_radar(fig, ax_map, lon, lat, region_name, data):

	display_coord = ax_map.projection.transform_point(lon, lat, ccrs.PlateCarree())
	ax_pos = ax_map.transData.transform(display_coord)
	inv = fig.transFigure.inverted()
	fig_coords = inv.transform(ax_pos)

	size = 0.1
	inset_ax = fig.add_axes([fig_coords[0] - size/2, fig_coords[1] - size/2, size, size], polar=True)

	theta = np.linspace(0, 2*np.pi, n_models, endpoint=False)
	bars = inset_ax.bar(theta, data, width=2*np.pi/n_models, color=colors, edgecolor='white')

	inset_ax.set_xticks([])
	inset_ax.set_yticks([])
	inset_ax.set_ylim(0, max(data)+1)
	inset_ax.set_title(region_name, fontsize=10, fontweight='bold', y=1.1)


# Cria figura e mapa base
fig = plt.figure(figsize=(14, 12))
ax = plt.axes([0.05, 0.05, 0.85, 0.9], projection=ccrs.PlateCarree())
ax.set_extent([-82, -32, -40, 10], crs=ccrs.PlateCarree())
...
# Adiciona os radares para cada região
for region, (lon, lat) in region_coords.items():
    ranks = region_ranks[region]
    add_radar(fig, ax, lon, lat, region, ranks)

# Cria legenda com as cores dos modelos
legend_elements = [Patch(facecolor=colors[i], edgecolor='white', label=model_names[i]) for i in range(n_models)]
ax_legend = fig.add_axes([0.72, 0.1, 0.25, 0.29])
ax_legend.axis('off')
ax_legend.legend(handles=legend_elements, loc='center left', fontsize=9)
